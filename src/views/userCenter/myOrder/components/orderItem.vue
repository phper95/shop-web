        ...
        <ul class="ListItem" v-for="(item,index1) in this.list" :key="index1">
            <div class="ordertitle" >
                <div class="title">
                    ...
                    <div class="titleTime">下单时间：{{ item.create_time | comverTime('YYYY-MM-DD HH:mm:ss')}}</div>
                    <div class="titleTime">订单号：{{ item.display_order_id }}</div>
                    <div class="titleTime" v-if="item.status === 10">付款：<span>{{ item.resetTime }}</span></div>
                </div>
                <div class="deleBtn" @click="deleOrderBtn(item.unique, item.order_id)"><svg t="1618218277357" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1480" width="14" height="14"><path d="M798.1 872.6c0 34.3-27.9 62.2-62.1 62.2H288.1c-34.3-0.1-62.1-27.9-62.2-62.2V212.8h572.2v659.8zM350.2 101.2c0-7.2 5.6-12.8 12.8-12.8h298.8c7.2 0 12.7 5.6 12.7 12.8v37.5H350.2v-37.5z m634.3 37.5H748.7v-37.5c0-47.8-39-86.9-86.9-86.9H363c-47.9 0.1-86.8 38.9-86.9 86.9v37.5H39.5C18.7 138.7 2 155.4 2 176.1s16.7 37.5 37.5 37.5H151v659c0 75.7 61.4 137.1 137.1 137.1h447.8c75.7 0 137.1-61.4 137.1-137.1V212.8h111.6c20.7 0 37.5-16.7 37.5-37.5s-16.8-36.6-37.6-36.6zM512 822.4c20.7 0 37.5-16.7 37.5-37.5V386.5c0-20.7-16.7-37.5-37.5-37.5-20.7 0-37.5 16.7-37.5 37.5v398.4c0 20.7 16.8 37.5 37.5 37.5m-174.5 0c20.7 0 37.5-16.7 37.5-37.5V386.5c0-20.7-16.7-37.5-37.5-37.5-20.7 0-37.5 16.7-37.5 37.5v398.4c0.8 20.7 17.6 37.5 37.5 37.5m349 0c20.7 0 37.5-16.7 37.5-37.5V386.5c0-20.7-16.7-37.5-37.5-37.5-20.7 0-37.5 16.7-37.5 37.5v398.4c0.1 20.7 16.8 37.5 37.5 37.5" p-id="1481"></path></svg>删除</div>
            </div>
            <div class="orderContent">
                <div class="commoditys">
                    <div class="orderCommodity" v-for="(i,index2) in item.cart_info" :key="index2">
                        <div class="orderPre">
                            <div class="img">
                                <!-- <img :src="i.img" /> -->
                ...
                <div class="orderStatusBtnList">
                    <div class="column">
                        ...
                        v-if="item.status === 11"
                        @click="invitBtn(item.pink_id)">邀请好友</button>
                        <!-- <button
                        class="orderBtn remind"
                        ...
                        class="orderBtn remind"
                        v-if="item.status === 1"
                        @click="takeBtn(item.id, item.order_id)">确认收货</button>
                        <!-- <button
                        class="orderBtn normal"
                        v-if="item.status === 0 ||item.status === 10"
                        @click="canOrderBtn(item.id, item.order_id)"
                        >取消订单</button> -->
                        <button
                        class="orderBtn normal"
                        v-if="item.status === 0 || item.status === 10"
                        ...
                        class="orderBtn remind"
                        v-if="item.status === 2"
                        @click="toPageId('/evaluate', item.order_id)">评价</button>
                        <button
                        class="orderBtn normal"
                        @click="rebackOrder(item.order_id)"
                        v-if="item.status === -1"
                        >撤销申请</button>
                        <button
                        @click="toPageId('/asRecordDetail', item.id, item.order_id)"
                        v-if="item.status === -1 || item.status == -2"
                        >售后详情</button>
                        <button
                        v-else
                        @click="toPageId('/orderDetail', item.order_id)"
                        >订单详情</button>
                        <!-- <button
                        @click="toPageId('/orderDetail', item.order_id)"
                        >订单详情</button> -->
    ...
    methods: {
        timeLeft () {
            ...
            this.timer = setInterval(() => {
                    this.list.map(item => {
                       var  mytime = this.$moment(item.create_time).format('YYYY-MM-DD HH:mm:ss')
                    //console.log(mytime)
        ...
        deleOrderBtn (id, order_id) { this.$emit('del', {'show': true, 'id': id, 'order_id': order_id}) },
        canOrderBtn (id, order_id) { this.$emit('cancel', {'show': true, 'id': id, 'order_id': order_id}) },
        invitBtn (id) { this.$emit('invit', {'show': true, 'id': id}) },
        takeBtn (id, order_id) { this.$emit('take', {'show': true, 'id': id, 'order_id': order_id}) },
